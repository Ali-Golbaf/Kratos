...
class MaterialTest(object):
  ...
  def PrepareTestTriaxialHydro(self):
      if ( ( self.test_type == "Triaxial") or ( self.test_type == "Hydrostatic") ):
        ...
        cylinder_lateral_area = math.pi * self.diameter * self.height
        cylinder_section_area = math.pi * self.diameter * self.diameter * 0.25

        #Correction Coefs  area of particles in cylinder lateral + cos(45)*Area of particles in top corner area
        self.alpha_top = cylinder_section_area/(self.xtop_area + 0.70710678*self.xtopcorner_area)
        self.alpha_bot = cylinder_section_area/(self.xbot_area + 0.70710678*self.xbotcorner_area)
        self.alpha_lat = cylinder_lateral_area/(self.xlat_area + 0.70710678*self.xtopcorner_area + 0.70710678*self.xbotcorner_area)

        # fraction of cylinder , for EXXON quarter area 0.25
        if self.diameter == 0.0508:
            self.alpha_lat = self.alpha_lat * 0.25
            self.alpha_bot = self.alpha_bot * 0.25
            self.alpha_top = self.alpha_top * 0.25
  ...
  def CylinderSkinDetermination(self): #model_part, solver, DEM_parameters):
        ...
        for element in self.spheres_model_part.Elements:
            ...
            particle_cross_section = math.pi * r * r
            ...
            if ((x * x + z * z) >= ((d / 2 - eps * r) * (d / 2 - eps * r))):
                ...
                if ((y > eps * r) and (y < (h - eps * r))):
                    ...
                    xlat_area = xlat_area + particle_cross_section
            ...
            if ((y <= eps * r) or (y >= (h - eps * r))):
                ...
                if ((x * x + z * z) >= ((d / 2 - eps * r) * (d / 2 - eps * r))):
                    ...
                    if (y > h / 2):
                        ...
                        xtopcorner_area = xtopcorner_area + particle_cross_section

                    else:
                        self.XBOTCORNER.append(node)
                        xbotcorner_area = xbotcorner_area + particle_cross_section
                else:
                    if (y <= eps * r):
                        self.XBOT.append(node)
                        xbot_area = xbot_area + particle_cross_section

                    elif (y >= (h - eps * r)):
                        self.XTOP.append(node)
                        xtop_area = xtop_area + particle_cross_section
        #checks:
  ...
  def MeasureForcesAndPressure(self):
    ...
    if (self.test_type =="BTS"):
      ...
      self.total_stress_bts = 2.0*total_force_bts/(math.pi*self.height*self.diameter*1e6)
  ...
  def ApplyLateralPressure(self, Pressure, XLAT, XBOT, XTOP, XBOTCORNER, XTOPCORNER, alpha_top, alpha_bot, alpha_lat):
      ...
      for node in XLAT:
          ...
          values[0] = particle_cross_section * alpha_lat * Pressure * vect[0]
          values[1] = 0.0
          values[2] = particle_cross_section * alpha_lat * Pressure * vect[2]
      ...
      for node in XTOPCORNER:
          ...
          values[0] = particle_cross_section * alpha_lat * Pressure * vect[0] * 0.70710678
          values[1] = 0.0
          values[2] = particle_cross_section * alpha_lat * Pressure * vect[2] * 0.70710678
